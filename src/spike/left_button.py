...
class Left_button:
    ISDEBUG = True
    #If BUTTONSIMULATE is true it waits for a left button to be pressed via the console, if false it monitors the state of the BUTTONPIN
    BUTTONSIMULATE = True
    ...
    def __init__(self):
         ...
         if(self.ISDEBUG):print("Left button is initialised in debug mode. Simulation:",self.BUTTONSIMULATE, " Button type:",self.BUTTONTYPE ," change at spike.left_button.py ")
    ...
    def wait_until_pressed(self):
        if(self.ISDEBUG):print("wait_until_pressed: Waits until the left button is pressed.")
        if(self.BUTTONSIMULATE):
            if(self.BUTTONTYPE == "Random"):
                ...
                while time.ticks_ms() - self.time < 1000 * self.BUTTONPRESSTIME:
                    if(self.ISDEBUG):print("Time remaining until simulated left button press :", str(1000 * self.BUTTONPRESSTIME - (time.ticks_ms() - self.time)))
    ...
    def wait_until_released(self):
        ...
        if(self.BUTTONSIMULATE):
            if(self.BUTTONTYPE == "Random"):
                ...
                self.time = time.ticks_ms()
                while time.ticks_ms() - self.time < 1000 * self.BUTTONPRESSTIME:
                    if(self.ISDEBUG):print("Time remaining until simulated left button released :", str(1000 * self.BUTTONPRESSTIME - (time.ticks_ms() - self.time)))
            elif(self.BUTTONTYPE == "Consol"):
        ...
        if(self.BUTTONSIMULATE):
            if(self.BUTTONTYPE == "Random"):
                ...
                if time.ticks_ms() - self.time > 1000 * self.BUTTONPRESSTIME:
                    if(self.ISDEBUG):print("Time is up and the simulated left button is pressed time ago:", str((time.ticks_ms() - self.time)-1000 * self.BUTTONPRESSTIME))
                    self.time = time.ticks_ms()
                    return True
                else:
                    if(self.ISDEBUG):print("Simulated left button press will come in :", str(1000 * self.BUTTONPRESSTIME- (time.ticks_ms() - self.time)), " msec")
                    return False
